//create own browser page
const { test, expect, chromium } = require('@playwright/test');

test('Handle Pages/Windows', async () => {
            
  const browser=await chromium.launch()         //browser contain the multiple context
  const context=await browser.newContext()      //context contain the multiple pages

  const page1=await context.newPage()
  const page2=await context.newPage()

  const allPages=context.pages()            //return all the pages
